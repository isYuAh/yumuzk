...
<div v-show="!zks.showFullPlay" class="play forbidSelect">
  ...
  <Transition name="heightAnim">
    ...
    <div @click="zks.showFullPlay = true" v-show="zks.play.show_songface" class="songface">
    ...
    <div @click="changeProgress" @mousemove="changeProgressTip"  @mouseleave="closeProgressTip" class="progress">
        ...
        <div ref="progressFill" :style="{width: `${zks.play.progress}%`}" class="fill"></div>
    </div>
    <div ref="songInformation" class="songInformation">
        <div @click="zks.showFullPlay = true" class="title">{{ zks.play.song.title }}</div>
        <div class="singer">{{ zks.play.song.singer }}</div>
    </div>
    ...
    <div class="durationInfo">
        <div class="infoItem">{{ zks.play.curTime }} / {{ zks.play.durationTime }}</div>
    </div>
    <div class="volumeController">
        <div class="volumeTip">VOLUME {{ Math.round(100*zks.play.volume) }}</div>
        <div @click="changeVolume" class="volumeProgress">
            <div ref="volumeProgressFill" :style="{width: `${100*zks.play.volume}%`}" class="volumeProgressFill"></div>
        </div>
    </div>
    ...
    <div class="fullPlayBtn">
        <SonglistTooltip text="播放列表">
          <div @click="zks.showPlaylistSonglist = !zks.showPlaylistSonglist" class="fullPlayBtn">
...
function changeVolumeInfo() {
    if (songSource.value) {
        zks.value.play.volume = songSource.value.volume;
        config.value.volume = songSource.value.volume
...
function updateTime() {
    if (songSource.value) {
        ...
        zks.value.play.progress = minmax(c / zks.value.play.duration * 100, 0, 100);
        zks.value.play.curTimeNum = c
        zks.value.play.curTime = secondsToMmss(c);
        emitter.emit('updateActiveLrcIndex');
    }
}
function changeProgress(e: MouseEvent) {
    if (songSource.value) {
        songSource.value.currentTime = minmax(Math.round(zks.value.play.duration * e.clientX / document.body.clientWidth), 0, zks.value.play.duration);
    }
}
function changeProgressTip(e: MouseEvent) {
    ...
    progress_tooltip.value.innerText = secondsToMmss(minmax(Math.round(zks.value.play.duration * e.clientX / document.body.clientWidth), 0, zks.value.play.duration));
...
watchEffect(() => {
  if ('mediaSession' in navigator) {
    navigator.mediaSession.metadata!.title = zks.value.play.song.title,
    navigator.mediaSession.metadata!.artist = zks.value.play.song.singer,
    navigator.mediaSession.metadata!.artwork = [{
      src: zks.value.play.song.pic,
    }]
  }
})
    ...
    if (song.type === 'bilibili') {
        client.get('https://api.bilibili.com/x/web-interface/view', {
            ...
            client.get('https://api.bilibili.com/x/player/wbi/playurl', {
                ...
                if (songSource.value) {
                    zks.value.play.song.url = res.data.data.durl[0].url;
            ...
            if (res.data.data[0]) {
                if (songfaceImg.value) {
                    if (song.pic) {
                        ...
                        zks.value.play.show_songface = true;
                        zks.value.play.song.pic = res.data.data[0].pic;
        ...
        if (!config.value.neteaseApi.enable) {
          ...
          normalClient.get(config.value.neteaseApi.url + 'song/detail', {params: {ids: song.id}}).then((res: AxiosResponse) => {
            rawDetail = res.data;
            if (songfaceImg.value) {
              if (song.pic) {
                zks.value.play.show_songface = true;
            ...
            if (rawDetail.privileges[0].chargeInfoList[0].chargeType != 0 && neteaseUser.value.vipType == '0') {
              ...
              normalClient.get(config.value.neteaseApi.url + 'song/url', {params: {
                  id: song.id,
            ...
            if (zks.value.play.song.lrc.status === 'disabled' || zks.value.play.song.translationLrc.status === 'disabled') {
                normalClient.get(config.value.neteaseApi.url + 'lyric', {params: {id: song.id}}).then((res: AxiosResponse) => {
...
function deleteSongInPlaylistSonglist(index?: number | object) {
  ...
  zks.value.play.playlist.splice(index, 1);
...
watch(() => zks.value.play.show_songface, (nv) => {
