...
<div class="partContainer forbidSelect">
    <div @click="goAndPlay(list)" v-for="list in useZKStore().lists" class="item">
        <TargetBorder>
            <div class="img">
...
import { clientInjectionKey } from '../types';
...
function goAndPlay(list: list) {
    ...
    if (list.type === 'data') {
        useZKStore().play.songs = list.songs;
        ZKStore.nowTab = 'songList';
        ...
        let getNextPage = function() {
            ...
            client.get('https://api.bilibili.com/x/v3/fav/resource/list', {
                ...
                ZKStore.play.songs.push(...res.data.data.medias.map((m: any) => ({
                    type: 'bilibili', 
                    BV: m.bvid, 
                    title: m.title,
                    pic: m.cover,
        ...
        normalClient.get(songsApi).then(res => {
            ZKStore.play.songs = res.data.data.list.map((s: any) => {
                return <song>{
                    title: s.name,
