...
<div class="partContainer">
    ...
    <LoadingMask :loading="loading" class="LoadingMask">
        <div class="songs">
            <div class="container">
                <simplebar data-auto-hide class="simplebar">
                    <div class="songTable forbidSelect">
                        <div
                            @dblclick="emitter.emit('playSong', {song, justtry: true})"
                            class="song" 
        ...
        if (suggestSelected.value === -1) {
            ...
            normalClient.get(url, {
                ...
                if (res.data.result.songs) {
                    resultList.value = res.data.result.songs.map((song: any) => {
                        return <song>{
                            type: 'netease',
                            title: song.name,
                            id: song.id,
                            singer: song.artists.map((ar: any) => ar.name).join(' & '),
                        }
                    })
